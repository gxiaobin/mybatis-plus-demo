...
public class FileUtil {
    ...
    public static byte[] toByteArray(InputStream in) throws IOException { return toByteArray(in, Integer.MAX_VALUE); }
    public static byte[] toByteArray(InputStream in, int maxSize) throws IOException {
        ...
        while (true) {
            if (len >= bytesSize) {// 达到数组上限，翻倍
                bytesSize <<= 1;
                if (bytesSize > maxSize) bytesSize = maxSize + 8;
                if (bytesSize < 0) bytesSize = Integer.MAX_VALUE; // greater than max int
